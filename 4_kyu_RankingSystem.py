...
# next level
# Any remaining progress earned while in the previous rank will be applied towards the next rank's progress (we
# don't throw any progress away). The exception is if there is no other rank left to progress towards (Once you reach rank 8 there is no more progression).
# A user cannot progress beyond rank 8.
# The only acceptable range of rank values is -8,-7,-6,-5,-4,-3,-2,-1,1,2,3,4,5,6,7,8. Any other value should raise
# an error.
#
...
class InvalidRanking(Exception):
    "Raised when problem ranking is outside of valid range"
    def __init__(self, problem_ranking, message="Ranking is not in (-8, 8) range or is zero"):
        self.ranking = problem_ranking
...
class User:
    ...
    def rank_adjustment (self):
        ...
        if self.progress >= 100:
            self.rank = self.valid_ranks[self.valid_ranks.index(self.rank) + 1]
            self.progress -= 100
        else:
            return
    ...
    def inc_progress(self, problem_rank):
        ...
        if not (problem_rank in self.valid_ranks):
            raise(InvalidRanking(problem_rank))
